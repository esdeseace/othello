...
function start() {
  ...
  for (var i = 0; i < players.length; ++i) {
    ...
    if (players.item(i).checked) {
      let choice = players.item(i).value;
      if (choice === "white_first") {
        player = 0; // Black (you) go first
        ai_player = 1;
      } else if (choice === "white_first_ai") {
        player = 0; // Black (AI) go first
        ai_player = 0;
      } else if (choice === "black_first") {
        player = 1; // White (you) go first
        ai_player = 0;
      } else if (choice === "black_first_ai") {
        player = 1; // White (AI) go first
        ai_player = 1;
      }
    }
  }

  console.log("player starts as", player === 0 ? "Black" : "White");
  console.log("AI plays as", ai_player === 0 ? "Black" : "White");
...
function show(r, c) {
    ...
    for (var x = 0; x < 8; ++x) {
      ...
      if (grid[y][x] == 0) {
        ...
        if (r == -1 || inside(r, c)) {
          ...
          document
            .getElementById(cell_id)
            .setAttribute("onclick", "move(" + y + ", " + x + ")");
        } else {
          document.getElementById(stone_id).className = "empty_stone";
          document.getElementById(cell_id).setAttribute("onclick", "");
        }
  ...
  for (var y = 0; y < hw; ++y) {
    for (var x = 0; x < hw; ++x) {
      if (grid[y][x] == 0) ++black_count;
      else if (grid[y][x] == 1) ++white_count;
    }
  }
...
async function ai() {
  ...
  for (var y = 0; y < hw; ++y) {
    for (var x = 0; x < hw; ++x) {
      if (grid[y][x] == 0) res[y * hw + x] = 0;
      else if (grid[y][x] == 1) res[y * hw + x] = 1;
      else res[y * hw + x] = -1;
    }
  }
...
function calc_value() {
  ...
  for (var y = 0; y < hw; ++y) {
    for (var x = 0; x < hw; ++x) {
      if (grid[y][x] == 2) {
        if (
          -64 <= output_array[10 + y * hw + x] &&
          output_array[10 + y * hw + x] <= 64
        ) {
          var stone_id = "stone_" + (y * hw + x);
          document.getElementById(stone_id).innerHTML =
            output_array[10 + y * hw + x];
        }
      }
    }
  }
...
function update_record() {
  ...
  var new_coord =
    String.fromCharCode(97 + record[record.length - 1][1]) +
    String.fromCharCode(49 + record[record.length - 1][0]);
...
function end_game() {
  ...
  if (show_value) hint = lang_with_hint;
  if (stones[ai_player] < stones[1 - ai_player]) {
    ...
    tweet_str =
      ...
      dis +
      lang_tweet_str_4 +
      lang_tweet_str_5_lose;
  } else {
    document.getElementById("result_text").innerHTML = lang_draw;
    tweet_str =
      lang_tweet_str_0_draw +
      lang_tweet_str_1 +
      level_names[level_idx] +
      lang_tweet_str_2 +
      hint +
      lang_tweet_str_4 +
      lang_tweet_str_5_draw;
  }
  ...
  tweet_result.innerHTML =
    ...
    tweet_str +
    '" data-url="https://www.egaroucid.nyanyan.dev/ja/web/" data-hashtags="egaroucid" data-related="takuto_yamana" data-show-count="false">Tweet</a><script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>';
  twttr.widgets.load();
  ...
  let players = document.getElementsByName("ai_player");
  for (var i = 0; i < 2; ++i) players.item(i).disabled = false;
}
